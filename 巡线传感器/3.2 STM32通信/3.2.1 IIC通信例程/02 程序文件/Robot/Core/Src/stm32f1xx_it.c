...
void SysTick_Handler(void)
{
  ...
  /* USER CODE BEGIN SysTick_IRQn 1 */
	...
	if(sys_count % 1000 == 0){
		if(get_battery_volocity() < 6.5){
			buzzer_toggle(400, 200, 1);
		}
	}
...
void USART2_IRQHandler(void)
{
  /* USER CODE BEGIN USART2_IRQn 0 */
		#ifdef __LINEFOLLOW_H_
    if (__HAL_UART_GET_FLAG(&huart2, UART_FLAG_TXE) != RESET)
		{
        __HAL_UART_CLEAR_FLAG(&huart2, UART_FLAG_TXE);
				/* 判断数据是否发送完毕 */
        if (LineFollow.tx_byte_index < 1) 
				{  
						LineFollow.it_state = LINEFOLLOW_WRITE_DATA;
						huart2.Instance->DR = ((uint8_t*)(&LineFollow.tx_frame))[LineFollow.tx_byte_index++]; /* 继续发送下一个字节 */
        } 
				else 
				{
    ...
    if(__HAL_UART_GET_FLAG(&huart2, UART_FLAG_TC) != RESET) 
        ...
        if (LineFollow.tx_only)
				{
            LineFollow.it_state = LINEFOLLOW_WRITE_DATA_READY;
        }
				else
				{
						LineFollow.it_state = LINEFOLLOW_READ_DATA;
    ...
    if(__HAL_UART_GET_FLAG(&huart2, UART_FLAG_RXNE) != RESET) 
        ...
        if (0 == LineFollow_rx_handler(&LineFollow, (uint8_t)(huart2.Instance->DR & (uint8_t)0x00FF)))
				{
						LineFollow.it_state = LINEFOLLOW_READ_DATA_FINISH;
        }
    }
	#else
		if (__HAL_UART_GET_FLAG(&huart2, UART_FLAG_TXE) != RESET)
		{	
			...
			{  
				...
				huart2.Instance->DR = ((uint8_t*)(&serial_servo_controller.tx_frame))[serial_servo_controller.tx_byte_index++]; /* 继续发送下一个字节 */
			} 
			else 
			{
				__HAL_UART_DISABLE_IT(&huart2, UART_IT_TXE);
